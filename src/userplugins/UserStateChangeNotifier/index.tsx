...
function triggerVoiceNotification(userId: string, userChannelId: string | null) {
    ...
    const title = shouldBeNative() ? `[Paradoxon] User ${name} changed voice status` : "User voice status change";
    if (userChannelId) {
        if (userChannelId !== myChanId) {
            showNotification({
                ...
                richBody: getRichBody(user, `[Paradoxon] ${name} joined a new voice channel`),
            });
        }
    } else {
        showNotification({
            ...
            noPersist: !settings.store.persistNotifications,
            richBody: getRichBody(user, `[Paradoxon] ${name} left their voice channel`),
        });
    }
}
        ...
        PRESENCE_UPDATES({ updates }: { updates: PresenceUpdate[]; }) {
            ...
            for (const { user: { id: userId, username }, status, clientStatus } of updates) {
                ...
                if (lastStatuses.has(userId) && lastStatuses.get(userId) !== status) {
                    ...
                    showNotification({
                        title: shouldBeNative() ? `[Paradoxon] User Status Change` : "User status change",
                        body: `${name}'s status is now: [${status}]`,
                        noPersist: !settings.store.persistNotifications,
                        richBody: getRichBody(user, `[Paradoxon] ${name}'s status is now ${status}`),
                    });
                }
